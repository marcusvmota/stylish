/**
 * Monochromatic WebGL fluid cursor.
 * Adapted from Pavel Dobryakov's WebGL Fluid Simulation (MIT) — stripped down
 * and recolored to white/grey so it fits a black & white identity. The canvas
 * clears to black and paints white dye; composite it with CSS `mix-blend: screen`
 * so the black vanishes and only the bright wisps glow over the page.
 */
export function fluidCursor(canvas: HTMLCanvasElement) {
  const config = {
    SIM_RESOLUTION: 128,
    DYE_RESOLUTION: 1024,
  ...
  function addKeywords(source: string, keywords?: string[]) {
    ...
    let k = ''
    keywords.forEach((kw) => (k += '#define ' + kw + '\n'))
    return k + source
  }
  ...
  const displayProgram = new Program(
    baseVertexShader,
    compileShader(gl!.FRAGMENT_SHADER, displayShaderSource, config.SHADING ? ['SHADING'] : undefined),
  )
  ...
  // Monochrome dye — white/grey wisps to fit the black & white identity
  function monoColor() {
    const v = 0.22 + Math.random() * 0.12
    return { r: v, g: v, b: v }
  }
  ...
  const onMove = (e: MouseEvent) => {
    ...
    updatePointerMoveData(pointer, posX, posY, monoColor())
  }
  window.addEventListener('mousemove', onMove)

  let raf = requestAnimationFrame(updateFrame)
